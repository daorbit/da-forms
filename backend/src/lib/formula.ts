 ...
 *
 * A hand-written parser rather than `eval` or `new Function`. The expression is
 * written by whoever built the form, but it is *evaluated* on our server for
 * every respondent — so an author who pasted something they did not understand,
 * or a form saved through a compromised builder session, would be running that
 * text in the same process as the database credentials. A parser that
 * understands five operators and nothing else cannot be talked into anything
 * else.
 *
 * Deliberately small: numbers, `{{Field Label}}` references, `+ - * / %`,
 * parentheses. No function calls, no property access, no strings, no
 * assignment. Anything outside that grammar is a formula that does not compile,
 * which the builder shows as an error rather than shipping to respondents.
 */
